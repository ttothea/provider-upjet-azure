...
// SetupWebhookWithManager registers the conversion webhook for MaintenanceAssignmentDedicatedHost.
func SetupWebhookWithManager(mgr ctrl.Manager) error {
	if err := ctrl.NewWebhookManagedBy(mgr, &v1beta1.MaintenanceAssignmentDedicatedHost{}).
		Complete(); err != nil {
...
// SetupGated adds a controller that reconciles MaintenanceAssignmentDedicatedHost managed resources.
func SetupGated(mgr ctrl.Manager, o tjcontroller.Options) error {
	o.Options.Gate.Register(func() {
		if err := Setup(mgr, o); err != nil {
			mgr.GetLogger().Error(err, "unable to setup reconciler", "gvk", v1beta1.MaintenanceAssignmentDedicatedHost_GroupVersionKind.String())
		}
	}, v1beta1.MaintenanceAssignmentDedicatedHost_GroupVersionKind)
	return nil
}

// Setup adds a controller that reconciles MaintenanceAssignmentDedicatedHost managed resources.
func Setup(mgr ctrl.Manager, o tjcontroller.Options) error {
	name := managed.ControllerName(v1beta1.MaintenanceAssignmentDedicatedHost_GroupVersionKind.String())
	var initializers managed.InitializerChain
	rl := reconciliationpolicy.NewExponentialFailureRateLimiter(time.Second, 60*time.Second)
	eventHandler := handler.NewEventHandler(handler.WithDefaultRateLimiter(rl), handler.WithLogger(o.Logger.WithValues("gvk", v1beta1.MaintenanceAssignmentDedicatedHost_GroupVersionKind)))
	ac := tjcontroller.NewAPICallbacks(mgr, xpresource.ManagedKind(v1beta1.MaintenanceAssignmentDedicatedHost_GroupVersionKind), tjcontroller.WithEventHandler(eventHandler), tjcontroller.WithStatusUpdates(false))
	opts := []managed.ReconcilerOption{
		managed.WithExternalConnecter(
			tjcontroller.NewTerraformPluginSDKAsyncConnector(mgr.GetClient(), o.OperationTrackerStore, o.SetupFn, o.Provider.Resources["azurerm_maintenance_assignment_dedicated_host"],
				...
				tjcontroller.WithTerraformPluginSDKAsyncMetricRecorder(metrics.NewMetricRecorder(v1beta1.MaintenanceAssignmentDedicatedHost_GroupVersionKind, mgr, o.PollInterval)),
	...
	r := managed.NewReconciler(mgr, xpresource.ManagedKind(v1beta1.MaintenanceAssignmentDedicatedHost_GroupVersionKind), opts...)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&v1beta1.MaintenanceAssignmentDedicatedHost{}, eventHandler).
		Complete(
